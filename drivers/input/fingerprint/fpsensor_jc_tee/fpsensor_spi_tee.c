...
int fpsensor_spidev_dts_init(fpsensor_data_t *fpsensor)
{
    ...
    if (node) {
        ...
        if (IS_ERR(fpsensor->fp_power_low)) {
            ...
            fpsensor_error("fpsensor Cannot find fp pinctrl fp_power_low!\n");
            return ret;
        }
        ...
        if (IS_ERR(fpsensor->fp_power_high)) {
            ...
            fpsensor_error( "fpsensor Cannot find fp pinctrl fp_power_high!\n");
            return ret;
        }
...
/*add eint*/
      finger_int_pin = of_get_named_gpio_flags(node,
                        "fingerprint,touch-int-gpio", 0, NULL);
    if (!gpio_is_valid(finger_int_pin)){
        printk("invalid irq gpio!");
  //      return -EINVAL;
    }

        fpsensor->irq = gpio_to_irq(finger_int_pin);
        fpsensor_printk(" [fpsensor23]fpsensor->irq= %d,fpsensor>irq_gpio = %d \n", fpsensor->irq,
                        fpsensor->irq_gpio);
    } else {
...
static int read_chipid(void) {
    ...
    uint8_t buf[4] = {0x08, 0x55, 0x00, 0x00 }, rbuf[4];
    ...
    for(i=0; i<4; i++){       
        ...
        buf[3]=0x00;
        r = tee_spi_transfer((void *)&smt_conf, sizeof(smt_conf), buf, rbuf, 4);
        printk("fpsensor read_chipid rbuf[0]%x rbuf[1]%x rbuf[2]%x rbuf[3]%x \n ",rbuf[0],rbuf[1],rbuf[2],rbuf[3]);
