...
struct pt_device_access_data {
	...
	u8 fw_self_test_param[PT_MAX_SELF_TEST_PARAM];
...
/*******************************************************************************
 * FUNCTION: pt_cmcp_threshold_loading_show
 *
 * SUMMARY: The show method for cmcp_threshold_loading sysfs node. This node
 ...
 ******************************************************************************/
static ssize_t pt_cmcp_threshold_loading_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
static DEVICE_ATTR(cmcp_threshold_loading, S_IRUSR | S_IWUSR,
	pt_cmcp_threshold_loading_show,
	pt_cmcp_threshold_loading_store);
...
put_pm_runtime:
	...
	if (cd->show_timestamp)
		print_idx += scnprintf(buf_out, TTHE_TUNER_MAX_BUF,
			"[%u] SCAN_DATA:", jiffies_to_msecs(jiffies));
	else
		print_idx += scnprintf(buf_out, TTHE_TUNER_MAX_BUF,
			"SCAN_DATA:");
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	struct pt_device_access_data *dad = data->dad;
	ssize_t length;
	u32 input_data[PT_MAX_PIP_MSG_SIZE];
	int rc = 0;
	int i;
	...
	if (length == 1) {
		...
		dad->fw_self_test_param_len = 0;
	} else if (length > 2 && ((length - 2) < PT_MAX_SELF_TEST_PARAM)) {
		dad->fw_self_test_id = input_data[0];
		dad->fw_self_test_format = input_data[1];
...
put_runtime:
	...
	if (cd->show_timestamp)
		print_idx += scnprintf(buf_out, TTHE_TUNER_MAX_BUF,
			"[%u] PT_DATA:", jiffies_to_msecs(jiffies));
	else
		print_idx += scnprintf(buf_out, TTHE_TUNER_MAX_BUF,
			"PT_DATA:");
	for (i = 0; i < length; i++)
...
/*******************************************************************************
 * FUNCTION: pt_setup_sysfs_attention
 *
 * SUMMARY: Funtion to be registered to TTDL attention list to setup sysfs for
 *  device test.
 *
 * RETURN:
 *	 0 = success
 ...
 *	*dev  - pointer to device structure
 ******************************************************************************/
static int pt_setup_sysfs_attention(struct device *dev)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
	int rc = 0;
	...
	cmd->unsubscribe_attention(dev, PT_ATTEN_STARTUP,
		PT_DEVICE_ACCESS_NAME, pt_setup_sysfs_attention,
		0);
	...
	*data = dad;
	...
	if (dad->si) {
		...
		cmd->subscribe_attention(dev, PT_ATTEN_STARTUP,
			...
			pt_setup_sysfs_attention, 0);
	}

	INIT_WORK(&dad->cmcp_threshold_update,
		pt_parse_cmcp_threshold_builtin);
