...
#define PT_LDR_REQUEST_EXCLUSIVE_TIMEOUT		1000
...
struct pt_loader_data {
	...
	struct work_struct bl_from_file_work;
#endif
};
		...
		bool forced)
{
	...
	if (!ld->loader_pdata) {
		update_fw_status = UPDATE_FW_NO_PLATFORM_DATA;
		pt_debug(dev, DL_ERROR,
	...
	if (!fw || !fw->img || !fw->size) {
		update_fw_status = UPDATE_FW_NO_FW_PROVIDED;
		pt_debug(dev, DL_ERROR,
	...
	if (!fw->ver || !fw->vsize) {
		update_fw_status = UPDATE_FW_INVALID_FW_IMAGE;
	...
	if (upgrade)
		return pt_upgrade_firmware(dev,
			fw->img, fw->size, &update_fw_status);
	else
		update_fw_status = UPDATE_FW_IDLE;
...
static void _pt_firmware_cont(const struct firmware *fw, void *context)
{
	...
	update_fw_status = UPDATE_FW_ACTIVE_1;
	if (!fw) {
		update_fw_status = UPDATE_FW_NO_FW_PROVIDED;
...
#else
	retval = request_firmware_direct(&fw_entry, filename, dev);
#endif
	if (retval < 0) {
...
/*******************************************************************************
 * FUNCTION: _pt_open_us_file
 *
 * SUMMARY: Open a user space file and store the contents in buf
 ...
 ******************************************************************************/
static int _pt_open_us_file(struct device *dev, u8 *file_path, u8 *buf,
	int *size)
{
	...
	if (file_len > PT_PIP2_MAX_FILE_SIZE) {
	...
	if (vfs_read(filp, buf, file_len, &(filp->f_pos)) != file_len) {
		...
		void *context)
{
	...
	if (!fw) {
		if (ld->pip2_load_builtin) {
			...
			ret = _pt_open_us_file(dev, cd->pip2_us_file_path,
				fw_img, &fw_size);
			if (ret) {
				pt_debug(dev, DL_ERROR,
	...
	if (ret) {
		...
		update_fw_status = UPDATE_FW_ENTER_BL_ERROR;
		goto exit;
	}
	update_fw_status += 1;
	...
	if (ret) {
		pt_debug(dev, DL_ERROR, "%s: Failed to get PIP Version\n",
			__func__);
		update_fw_status = UPDATE_FW_PIP_VERSION_ERROR;
		goto exit;
	}
	...
	if (ret < 0) {
		pt_debug(dev, DL_ERROR, "%s Open file %d failed\n",
			__func__, ld->pip2_load_file_no);
		update_fw_status = UPDATE_FW_FILE_OPEN_ERROR;
		goto exit;
	}
	...
	if (ld->pip2_load_file_no == PIP2_FW_FILE) {
		...
		if (ret < 0) {
			...
			update_fw_status = UPDATE_FW_ERASE_ERROR;
	...
	while (remain_bytes > PIP2_FILE_WRITE_LEN_PER_PACKET) {
		...
		/* Don't update BL status on every pass */
		if (remain_bytes % 2000 < PIP2_FILE_WRITE_LEN_PER_PACKET) {
			/* Calculate % complete for update_fw_status sysfs */
			percent_cmplt = (fw_size - remain_bytes) *
					100 / fw_size;
			update_fw_status = (percent_cmplt >
				update_fw_status) ? percent_cmplt :
				update_fw_status;
		...
		if (!ret && status) {
			...
			update_fw_status = UPDATE_FW_WRITE_ERROR;
			pt_debug(dev, DL_ERROR,
	...
	if (ret != ld->pip2_load_file_no) {
		...
		update_fw_status = UPDATE_FW_FILE_CLOSE_ERROR;
		goto exit;
	}
	update_fw_status = UPDATE_FW_ACTIVE_99;
	...
	if (ld->pip2_load_file_no == PIP2_RAM_FILE &&
		...
		if (ret || status) {
			...
			update_fw_status = UPDATE_FW_EXECUTE_ERROR;
			goto exit;
		}
		...
		if (pip_ver_major >= 2 && pip_ver_minor >= 3) {
			...
			cd->startup_status = STARTUP_STATUS_START;
			pt_debug(dev, DL_WARN, "%s: Launch APP now...\n",
				__func__);
	...
	pt_debug(dev, DL_WARN, "%s: APP launched\n", __func__);
	...
	if (cd->startup_status & STARTUP_STATUS_FW_RESET_SENTINEL) {
		ret = cmd->request_pip2_get_mode(dev, PT_CORE_CMD_UNPROTECTED,
			&mode, &sys_mode);
		pt_debug(dev, DL_WARN, "%s: mode = %d (Expected APP [2] )",
			__func__, mode);
		if (mode != PT_MODE_OPERATIONAL) {
			pt_debug(dev, DL_ERROR,
				"%s ERROR: Not in App mode as expected\n",
				__func__);
			update_fw_status = UPDATE_FW_MODE_ERROR;
			goto exit;
		}
	} else {
		...
		update_fw_status = UPDATE_FW_SENTINEL_NOT_SEEN;
		goto exit;
	}
...
exit:
	...
	if ((update_fw_status == UPDATE_FW_COMPLETE) ||
	    (update_fw_status == UPDATE_FW_VERSION_ERROR)) {
		pt_debug(dev, DL_WARN,
			 "%s Requesting ENUM, update fw status(%d)\n", __func__,
			 update_fw_status);
		cmd->request_enum(dev, true);
	}
...
#else
	retval = request_firmware_direct(&fw_entry, PT_FW_RAM_FILE_NAME, dev);
#endif
	if (retval < 0) {
...
/*******************************************************************************
 * FUNCTION: pt_bl_from_file_work_function
 *
 * SUMMARY: The work function to schedule the BL work
 *
 * PARAMETERS:
 *	*bl_from_file_work - pointer to work_struct structure
 ******************************************************************************/
static void pt_bl_from_file_work_function(struct work_struct *bl_from_file_work)
{
	struct pt_loader_data *ld = container_of(bl_from_file_work,
			struct pt_loader_data, bl_from_file_work);
	struct device *dev = ld->dev;
	const struct firmware *fw_entry = NULL;
 ...
 *
 * SUMMARY: The show method for the "pip2_bl_from_file" sysfs node.
 *
 * PARAMETERS:
 *	*dev  - pointer to device structure
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_unlock(&cd->firmware_class_lock);
	schedule_work(&ld->bl_from_file_work);

	return snprintf(buf, PT_MAX_PRBUF_SIZE,
		"Status: %d\n"
		"File Loading to DUT: %s\n",
		rc, cd->pip2_us_file_path);
}
/*******************************************************************************
 * FUNCTION: pt_pip2_bl_from_file_store
 *
 * SUMMARY: The show method for the "pip2_bl_from_file" sysfs node. Used to
 *	allow any file path to be used to BL. NOTE: the last char in buf
 *	which is a '\n' is not copied.
 *
 * PARAMETERS:
 *      *dev   - pointer to device structure
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size <= PT_MAX_PATH_SIZE) {
		...
		memcpy(cd->pip2_us_file_path, buf, size - 1);
	} else {
		memset(cd->pip2_us_file_path, 0, PT_MAX_PATH_SIZE);
	}
		...
		char *buf, loff_t offset, size_t count)
{
	...
	for (i = 0; i < read_size; i++)
		print_idx += scnprintf(pr_buf + print_idx,
			PIPE_BUF - print_idx,
			"%02X ", read_buf[i + PIP2_RESPONSE_BODY_OFFSET]);
 ...
 *
 *	NOTE: For parts that do not support the RUNFW pin (TT7000-A1, TC3315),
 *	the GET_LAST_ERROR command will always report a "0" in this byte.
 *
 * PARAMETERS:
 *	*dev  - pointer to device structure
 *	*attr - pointer to device attributes structure
...
exit_put_sync:
	...
	if (read_buf[PIP2_RESPONSE_STATUS_OFFSET] == PIP2_RSP_ERR_NONE) {
		return snprintf(buf, PT_MAX_PRBUF_SIZE,
			"Status: %d\n"
		...
		return snprintf(buf, PT_MAX_PRBUF_SIZE,
			...
			"Last Error No: n/a\n"
			"RUNFW State  : n/a\n",
			read_buf[PIP2_RESPONSE_STATUS_OFFSET]);
	}
}
...
static int pt_loader_probe(struct device *dev, void **data)
{
	...
	*data = ld;
#ifdef CONFIG_TOUCHSCREEN_PARADE_BINARY_FW_UPGRADE
	INIT_WORK(&ld->bl_from_file_work, pt_bl_from_file_work_function);
#endif
