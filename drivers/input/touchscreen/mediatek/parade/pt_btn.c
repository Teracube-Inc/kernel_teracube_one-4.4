...
#ifdef VERBOSE_DEBUG
/*******************************************************************************
 * FUNCTION: pt_log_btn_data
 *
 * SUMMARY: Prints all button information
 *
 * PARAMETERS:
 *     *bd  - pointer to button data structure
 ******************************************************************************/
static void pt_log_btn_data(struct pt_btn_data *bd)
{
	struct device *dev = bd->dev;
	...
	for (cur = 0; cur < si->num_btns; cur++) {
		...
		if (si->xy_data[0] & (1 << cur))
			...
			value = 0;
		snprintf(pr_buf, PT_MAX_PRBUF_SIZE, "btn_rec[%d]=0x", cur);
		snprintf(pr_buf, PT_MAX_PRBUF_SIZE, "%s%X (%02X)",
			pr_buf, value,
			le16_to_cpu(si->xy_data[1 + cur * 2]));

		pt_debug(dev, DL_INFO, "%s: %s\n", __func__, pr_buf);
	}
}
#endif
...
int pt_btn_release(struct device *dev)
{
	...
	if (bd->input_device_registered) {
		...
		bd->input_device_allocated = false;
	} else if (bd->input_device_allocated) {
		bd->input_device_allocated = false;
		input_free_device(bd->input);
