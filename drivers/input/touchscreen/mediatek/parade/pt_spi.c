...
exit:
	if (rc < 0)
		pt_debug(dev, DL_ERROR, "%s: spi_sync() error %d\n",
			__func__, rc);
#if 0 /* TC3315 - need to verify the ACK byte */
...
static int pt_spi_read_default_nosize(struct device *dev, u8 *buf, u32 max)
{
	u32 size;
	int rc;
	u8 tmp_buf[PT_MAX_PIP_MSG_SIZE];

	if (!buf)
		return 0;

	rc = pt_spi_xfer(dev, PT_SPI_RD_OP, tmp_buf, 3);
	if (rc < 0)
		return rc;

	size = get_unaligned_le16(&tmp_buf[1]);
	if (!size) {
		memcpy(buf, tmp_buf + 1, 2);
		return rc;
	}
	...
	rc = pt_spi_read_default(dev, tmp_buf, size + 1);
	if (!rc)
