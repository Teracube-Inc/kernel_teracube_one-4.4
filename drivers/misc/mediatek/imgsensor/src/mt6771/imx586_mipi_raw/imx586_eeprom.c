...
#define imx586_EEPROM_READ_ID  0xA1
#define imx586_EEPROM_WRITE_ID   0xA0
...
typedef struct IMX586_MIPI_otp_struct{
...
static kal_uint8 imx586_write_id = 0;
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE* data)
{
	...
	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, imx586_EEPROM_READ_ID)<0)
		return false;
    return true;
}
...
static kal_uint8 eeprom_read_byte(kal_uint32 addr)
{
    ...
    char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 1, imx586_EEPROM_READ_ID);
    return get_byte;
}

static bool _read_imx586_eeprom(kal_uint16 addr, BYTE* data, int size ){
	int i = 0;
	int offset = addr;
...
static void IMX586_MIPI_read_otp_wb(IMX586_OTP_TYPE *otp)
{	
    ...
    {
       LOG_INF("err : read imx586 eeprom no awb data\n");
    }	
}

void IMX586_MIPI_write_otp_wb(IMX586_OTP_TYPE *otp,kal_uint8 i2c_write_id)
{
    kal_uint16 RGr_ratio, BGr_ratio, GbGr_ratio;
    ...
    write_cmos_sensor_byte(0x020e,Gr_test>>8,imx586_write_id);
    write_cmos_sensor_byte(0x020F,Gr_test&0xff,imx586_write_id);
    write_cmos_sensor_byte(0x0210,R_test>>8,imx586_write_id);
    write_cmos_sensor_byte(0x0211,R_test&0xff,imx586_write_id);
    write_cmos_sensor_byte(0x0212,B_test>>8,imx586_write_id);
    write_cmos_sensor_byte(0x0213,B_test&0xff,imx586_write_id);
    write_cmos_sensor_byte(0x0214,Gb_test>>8,imx586_write_id);
    write_cmos_sensor_byte(0x0215,Gb_test&0xff,imx586_write_id);
	
    LOG_INF("IMX586_OTP:Gr_test=0x%x\n",Gr_test);
    LOG_INF("IMX586_OTP:R_test=0x%x\n",R_test);
    LOG_INF("IMX586_OTP:B_test=0x%x\n",B_test);
    LOG_INF("IMX586_OTP:Gb_test=0x%x\n",Gb_test);
    LOG_INF("IMX586_OTP:End.\n");
}

void IMX586_MIPI_update_awb(kal_uint8 i2c_write_id)
{
   IMX586_OTP_TYPE current_otp;  
   IMX586_MIPI_read_otp_wb(&current_otp);
   IMX586_MIPI_write_otp_wb(&current_otp,i2c_write_id);
}

bool imx586_read_otp_qsc(BYTE* data)
{
	int addr = 0x990;
	int size = 2304;
	int i = 0;	
	
	LOG_INF("read imx586 QSC, size = %d, get_done = %d, last_size = %d, last_offset = %d\n", size, get_done, last_size, last_offset);
	
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_imx586_eeprom(addr, data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
...
bool imx586_read_otp_lrc(BYTE* data)
{
	int addr = 0x7AF;
	int size = 384;
	int i = 0;	
	
	LOG_INF("read imx586 LRC, size = %d, get_done = %d, last_size = %d, last_offset = %d\n", size, get_done, last_size, last_offset);
...
bool Imx586CheckVersion(kal_uint32 imgsensor_writeid)
{
	...
	_read_imx586_eeprom(0x0000,&otp_flag,1);
	LOG_INF("read imx586 otp flag = %d\n", otp_flag);
	...
	_read_imx586_eeprom(0x0001,data,32);
