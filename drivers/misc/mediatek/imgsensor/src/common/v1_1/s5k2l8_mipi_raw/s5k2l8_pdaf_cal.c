...
#include "s5k2l8_pdaf_cal.h"


#include "s5k2l8mipiraw_Sensor.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "s5k2l8_camera_pdaf"
/* #define LOG_1 LOG_INF("s5k2l8,MIPI 4LANE\n") */
...
static bool s5k2l8_selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
...
static bool s5k2l8_read_eeprom(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	int i = 0;
	/* int offset = addr; */
	int offset = 0xec0;
	/* for(i = 0; i < 1404; i++) { */
	for (i = 0; i < 2048; i++) {
		if (!s5k2l8_selective_read_eeprom(offset, &data[i])) {
...
bool s5k2l8_read_otp_pdaf_data(kal_uint16 addr, BYTE *data, kal_uint32 size, kal_uint32 sensor_id)
{
	...
	if (!get_done || last_size != size || last_offset != addr) {
		if (!s5k2l8_read_eeprom(addr, data, size)) {
			get_done = 0;
			last_size = 0;
			last_offset = 0;
