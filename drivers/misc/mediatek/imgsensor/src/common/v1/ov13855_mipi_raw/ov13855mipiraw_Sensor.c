...
#define GPIO_GC0310_PDN           (23+343)
#define GPIO_GC0310_RST           (22+343)
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    gpio_set_value_cansleep(GPIO_GC0310_RST, 1);
...
int yuv_sensor_read_shutter(void)
{
    int val;
    val=GC0310_Read_Shutter();
    return val;   //val > 0  ata test suscess
}
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
    {
        ...
        //if (cnt > 0) 
        { 
     //       printk("yuv sensor open begin, count:%d\n", cnt);
            gpio_set_value_cansleep(GPIO_GC0310_PDN, 0);
            gpio_set_value_cansleep(GPIO_GC0310_RST, 1);

	        mdelay(5);
            gc0310open();
       //     printk("yuv sensor open end\n");
        }    
    }
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 close(void)
{
    LOG_INF("E\n");
        ...
        gpio_set_value_cansleep(GPIO_GC0310_RST, 0);
    return ERROR_NONE;
}   /*  close  */
