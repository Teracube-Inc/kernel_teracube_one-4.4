/* drivers/hwmon/mt6516/amit/mn58xxx.c - MN58xxx ALS/PS driver
 *
...
#define ALS_REGISTRIED  1
#define COMBINED_MN26xxx    0
#if COMBINED_MN26xxx
#include "mn26xxx.h"
extern int mn_als_open_report_data(int open);
extern int mn_als_enable_nodata(int en);
extern int mn_als_set_delay(u64 ns);
extern int mn_als_get_data(int* value, int* status);
#if ANDR_O
extern int mn_als_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs);
extern int mn_als_flush(void);
#endif
#endif
/******************************************************************************
 * driver info
*******************************************************************************/
#define EPL_DEV_NAME   		    "MN58xxx"
#define DRIVER_VERSION          "1.0.0_ANDR_O"
/******************************************************************************
 * ALS / PS sensor structure
*******************************************************************************/
#define COMPATIABLE_NAME "mediatek,mn58xxx"
static struct alsps_hw alsps_cust;
static struct alsps_hw *hw = &alsps_cust;
#if COMBINED_MN26xxx
#define COMPATIABLE_MN_NAME "mediatek,mn26xxx"
static struct alsps_hw *hw_mn = &alsps_cust;
#endif
...
static void write_global_variable(struct i2c_client *client)
{
    ...
    {
        epl_sensor_I2C_Write(client, 0xfb, 0x22) ;//mn58xxx only, others is 0x21
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.ps.ir_drive = EPL_IR_DRIVE_10;  //mn58xxx only support 10mA
    ...
    if(enable_als != enable)
    {
        if(enable)
        {
            ...
            if(epl_sensor.als.polling_mode == 0)
                set_lsensor_intr_threshold(epl_sensor.als.low_threshold, (epl_sensor.als.low_threshold+1) );
#if ALS_DYN_INTT
            if(epl_sensor.als.report_type == CMC_BIT_DYN_INT)
            {
                dynamic_intt_idx = dynamic_intt_init_idx;
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    int renvo = 0;
#if ALS_REGISTRIED || COMBINED_MN26xxx
#if COMBINED_MN26xxx
    const char *name_mn = COMPATIABLE_MN_NAME;
#endif
...
#if COMBINED_MN26xxx   //???
	err = get_alsps_dts_func(client->dev.of_node, hw_mn);
	if (err < 0)
    {
    	APS_ERR("[%s]: get dts info fail(mn)\n", __func__);
    }
#endif	/*COMBINED_MN26xxx*/
#endif
...
#elif COMBINED_MN26xxx
    als_ctl.open_report_data= mn_als_open_report_data;
	als_ctl.enable_nodata = mn_als_enable_nodata;
	als_ctl.set_delay  = mn_als_set_delay;
#if ANDR_O
	als_ctl.batch = mn_als_batch;
	als_ctl.flush = mn_als_flush;
#endif
	als_ctl.is_report_input_direct = obj->hw_mn->polling_mode_als ==0? true:false;
	als_ctl.is_polling_mode = obj->hw_mn->polling_mode_als==1? true:false;
#ifdef CUSTOM_KERNEL_SENSORHUB
	als_ctl.is_support_batch = true;
#else
    als_ctl.is_support_batch = false;
	...
	als_data.get_data = mn_als_get_data;
	als_data.vender_div = 100;
	err = als_register_data_path(&als_data);
	if(err)
	{
	...
	alsps_context_obj->ps_ctl.is_polling_mode = ps_ctl.is_polling_mode; //fixed ps polling_mode issue of alsps.c
#ifdef CUSTOM_KERNEL_SENSORHUB
...
#if COMBINED_MN26xxx
    hw_mn = get_alsps_dts_func(name_mn, hw);
    if (!name_mn)
    {
    	APS_ERR("[name_mn]: get dts info fail\n");
    }
#endif /*COMBINED_MN26xxx*/
#endif
...
MODULE_DESCRIPTION("MN58xxx ALPsr driver");
MODULE_LICENSE("GPL");
