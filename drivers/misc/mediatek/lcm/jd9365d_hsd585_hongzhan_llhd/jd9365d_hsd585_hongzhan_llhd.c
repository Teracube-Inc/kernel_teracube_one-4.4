...
static int adc_read_vol(void)
{
  ...
  adc_vol = sum/10;
#if defined(BUILD_LK)
  printf("jd9365 hongzhan adc_vol is %d\n",adc_vol);
#else
  printk("jd9365 hongzhan adc_vol is %d\n",adc_vol);
#endif
...
#if defined(BUILD_LK)
  printf("%s,jd9365 hongzhan id_high = 0x%08x,id = 0x%08x\n", __func__, id_high,id);
#else
  printk("%s,jd9365  hongzhan id_high= 0x%08x,id = 0x%08x\n", __func__, id_high,id);
#endif
